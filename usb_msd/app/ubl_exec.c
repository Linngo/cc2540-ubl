 ...
 ******************************************************************************
 ...
 PROVIDED “AS ISWITHOUT WARRANTY OF ANY KIND, EITHER EXPRESS OR IMPLIED,
...
static const uint8 aesKey[KEY_BLENGTH] = {
 ...
 // 0xab, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F
  'Y','0','T','p','a','F','u','r','Q','Y','g','L','I','!','f','5'
};
#endif
...
uint8 ublAesAuth(void)
{
  ...
  for (uint8 pgNum = UBL_PAGE_FIRST; pgNum <= UBL_PAGE_LAST; pgNum++)
  {
    ...
    for (uint16 oset = 0; oset < HAL_FLASH_PAGE_SIZE; )
    {
      if ((pgCnt == 1) && (oset == 0))
      {
        oset += KEY_BLENGTH;  // Must not include the signature bytes in the signature calculation.22;//
        pgBuf[21] = 0xff;
      }
...
/*#if defined UBL_GPIO_USE
	...
	if(stDelta/0x28000 == i) //(1 * 4096UL) / 125;  == 1ms
        {
          i++;
          P0_6 ^= 1;
        }
        if ((stDelta > stDelay) && (have_data == 0))
        {
          P1_0 = 0x0;
        }
      }
